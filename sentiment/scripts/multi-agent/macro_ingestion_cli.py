...
def run_macro_ingestion(
    ...
    nvidia_base_model = os.getenv("NVIDIA_BASE_MODEL_ALT", "").strip('"\'  ')
    ...
    def custom_macro_nested_chat_reply(chat_queue, recipient, messages, sender, config):
        # 1. First-Turn Loop Guard (prevent re-running nested chats on tool execution replies)
        if len(messages) > 1:
            return False, None
        ...
        # Helper to strip ReAct frameworks traces and extract raw final answer data
        def clean_react_summary(summary: str) -> str:
            if not summary:
                return ""
            summary = summary.strip()
            if "Final Answer:" in summary:
                parts = summary.split("Final Answer:")
        ...
        cleaned_forexfactory = clean_react_summary(forexfactory_summary)
        cleaned_alphavantage = clean_react_summary(alphavantage_summary)
        
        combined_summary = (
            f"Economic Calendar Events Data:\n{cleaned_forexfactory}\n\n"
            f"Historical Standard Deviation Data:\n{cleaned_alphavantage}"
        )
    ...
    # 9. Strip _scheduler audit block (if present) and route to audit logger
